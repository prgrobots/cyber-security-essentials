...
```mermaid
---
title: Authentication Mechanism Flowchart
---
flowchart TD
    A[User Login] --> B{Authentication Type?}
    B -- Multi-Factor --> C[Enter Password + OTP]
    B -- Password-Based --> D[Enter Password]
    B -- Biometric --> E[Scan Fingerprint]
    C --> F[Verify OTP]
    D --> G[Verify Password]
    E --> H[Verify Fingerprint]
    F --> I{OTP Valid?}
    G --> J{Password Valid?}
    H --> K{Fingerprint Valid?}
    I -- Yes --> L[Access Granted]
    I -- No --> M[Access Denied]
    J -- Yes --> N[Access Granted]
    J -- No --> O[Access Denied]
    K -- Yes --> P[Access Granted]
    K -- No --> Q[Access Denied]
```
```mermaid
---
...
---
graph TD
    A[User Interface] --> B{Authentication Type?}
    B -- Multi-Factor --> C[OTP Service]
    B -- Password-Based --> D[Password Storage]
    B -- Biometric --> E[Biometric Data Storage]
    C --> F[Verify OTP with Database]
    D --> G[Verify Password Hash]
    E --> H[Verify Fingerprint Match]
    A --> I[Submit Credentials]
    I --> J{OTP Submitted?}
    J -- Yes --> F
    J -- No --> K{Password Submitted?}
    K -- Yes --> G
    K -- No --> L{Biometric Data Submitted?}
    L -- Yes --> H
    L -- No --> M[Access Denied]
    C --> N{OTP Verified?}
    D --> O{Password Verified?}
    E --> P{Fingerprint Verified?}
    N -- Yes --> Q[Access Granted]
    N -- No --> R[Access Denied]
    O -- Yes --> S[Access Granted]
    O -- No --> T[Access Denied]
    P -- Yes --> U[Access Granted]
    P -- No --> V[Access Denied]
```
