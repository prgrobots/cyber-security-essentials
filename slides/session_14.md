...
```mermaid
---
title: Symmetric and Asymmetric Encryption Workflow
---
flowchart TD
    A[Data to Encrypt/Decrypt] -->|Symmetric (AES)| B[Encrypt with Key]
    B --> C[Encrypted Data]
    C --> D[Decrypt with Same Key]
    D --> E[Recover Original Data]
    F[Public/Private Keys] -->|Asymmetric (RSA)| G[Encrypt with Public Key]
    G --> H[Encrypted Data]
    H --> I[Decrypt with Private Key]
    I --> J[Recover Original Data]
```
```mermaid
---
title: Hashing Algorithm Workflow
---
flowchart TD
    A[Data to Hash] --> B[Hash Using SHA-256]
    B --> C[Hashed Value]
