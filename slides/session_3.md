...
```mermaid
---
title: Authentication Methods Flowchart
---
flowchart TD
    A[User Login] -->|Password-based authentication| B{Is password correct?}
    B -- Yes --> C[Access Granted]
    B -- No --> D[Access Denied]
    A -->|Token-based authentication| E{Is token valid?}
    E -- Yes --> F[Access Granted]
    E -- No --> G[Access Denied]
    A -->|Biometric authentication| H{Does biometric match?}
    H -- Yes --> I[Access Granted]
    H -- No --> J[Access Denied]
```
```mermaid
---
title: Role-Based Access Control (RBAC) Flowchart
---
flowchart TD
    A[User Logs In] --> B[Determine User Role]
    C{Is user role authorized for resource?}
    C -- Yes --> D[Access Granted]
    C -- No --> E[Access Denied]
    F[Resource Accessed] --> G[Monitor Access]
```
